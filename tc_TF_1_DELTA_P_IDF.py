...
class TDELTAIDF:
    ...
    def modified_idf_calculation(self, query_word):
        return math.log((self.no_of_documents + 1) / (self.no_of_documents_containing_a_word(query_word) + 0.5), math.exp(1))
    ...
    def t_delta_score(self, query, document_id):
        ...
        for key, value in query[2].items():
            ...
            score += self.modified_idf_calculation(key) * (1 + math.log(1 + math.log((word_freq_in_document / (1 - self.b + self.b * (document_length / self.average_length_of_all_documents))) + self.delta, math.exp(1)), math.exp(1)))
